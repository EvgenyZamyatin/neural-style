...
class CondInstanceNormalization(Layer):
    ...
    def build(self, input_shape):
        self.scale = self.add_weight(shape=(3, input_shape[0][1]), initializer="uniform", trainable=True)
        self.shift = self.add_weight(shape=(3, input_shape[0][1]), initializer="zero", trainable=True)
        super().build(input_shape)
