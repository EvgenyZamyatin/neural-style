...
if args.subcommand == "train":
    ...
    with tqdm(desc="Training", file=sys.stdout, ncols=100, total=args.train_iterations, ascii=True,
        ...
        for tri in range(args.train_iterations):
            ...
            alpha.set_value(np.random.randint(0, 2, (batch_size, 1)).astype(np.int32))
            ...
            if (tri + 1) % args.val_every == 0 or (tri + 1) == args.train_iterations:
                ...
                if args.test_image is not None:
                    X.set_value(test_image.repeat(3, axis=0), borrow=True)
                    alpha.set_value(np.arange(0, 3).reshape(3, 1).astype(np.int32))
                    test_tr = get_Xtr()
